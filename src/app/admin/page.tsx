...
import { 
  ...
  PieChart, DollarSign, Briefcase, MapPin, User, Save, Key, Bell, 
  ShieldCheck, Plus, FileText, UploadCloud, Edit3, Trash2, X, CheckCircle
} from 'lucide-react';
import Link from 'next/link';
// 🌟 引入我們剛剛建立好的獨立 CRM 組件
import CrmInvoiceModule from '@/components/CrmInvoiceModule';
...
export default function AdminPage() {
  ...
  const [capitalConfig, setCapitalConfig] = useState({
    principal: 100000000, interestRate: 8, durationYears: 5, upfrontFeeRate: 6     
  });
  ...
  useEffect(() => {
    ...
    if (savedNews) setNewsArticles(JSON.parse(savedNews));
  }, []);
  ...
  const handleGenerateNews = async () => {
    ...
    try {
      ...
      const newArticle = {
        ...
        date: new Date().toISOString().split('T')[0], summary: aiResult.summary || '摘要生成失敗',
        fullText: aiResult.fullText || '內文生成失敗', published: false
      };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col font-sans">
      ...
      <div className="flex flex-1 overflow-hidden print:hidden">
        ...
        <main className="flex-1 p-8 overflow-y-auto bg-slate-950 relative">
          ...
          {/* ================= 模組：CRM ================= */}
          {activeTab === 'crm' && <CrmInvoiceModule />}
