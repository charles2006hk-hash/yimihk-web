...
export default function NewsWidget() {
  const [selectedArticle, setSelectedArticle] = useState<typeof defaultNewsData[0] | null>(null);
  const [newsData, setNewsData] = useState(defaultNewsData);

  // 初始化：從 localStorage 讀取後台同步的數據
  useEffect(() => {
    const savedNews = localStorage.getItem('yimi_news_data');
    if (savedNews) {
      const parsedNews = JSON.parse(savedNews);
      // 只顯示狀態為 published: true 的文章
      setNewsData(parsedNews.filter((news: any) => news.published));
    }
  }, []);
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-3xl bg-slate-900 border border-slate-700 rounded-3xl p-8 md:p-10 shadow-2xl z-10 animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
            ...
            <div className="prose prose-invert max-w-none">
              <p className="text-slate-300 text-lg leading-relaxed whitespace-pre-wrap">{selectedArticle.fullText}</p>
            </div>
